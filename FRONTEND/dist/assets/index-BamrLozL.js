(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const u of o.addedNodes)u.tagName==="LINK"&&u.rel==="modulepreload"&&i(u)}).observe(document,{childList:!0,subtree:!0});function a(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(s){if(s.ep)return;s.ep=!0;const o=a(s);fetch(s.href,o)}})();class q{constructor(t){this.routes=t,this.currentRoute=null,window.addEventListener("hashchange",()=>this.handleRoute()),window.addEventListener("load",()=>this.handleRoute())}navigate(t){window.location.hash=t}handleRoute(){const t=window.location.hash.slice(1)||"/overview";let a=this.routes.find(i=>i.path===t);if(!a){for(const i of this.routes)if(i.path.includes(":")){const s=i.path.replace(/:(\w+)/g,"([^/]+)"),o=new RegExp(`^${s}$`),u=t.match(o);if(u){a={...i,params:{id:u[1]}};break}}}if(!a){this.navigate("/overview");return}this.currentRoute=t,this.onRouteChange&&this.onRouteChange(a,t)}getCurrentPath(){return this.currentRoute||"/overview"}}function K(e){return`
    ...
    `}function Y(){const e=document.getElementById("ticker-track");if(!e)return;const t=[{user:"@alex_ship",val1:"$1K",val2:"$2.5K"},{user:"@sarah_dev",val1:"$500",val2:"$1.2K"},{user:"@david_builds",val1:"$2K",val2:"$5.2K"},{user:"@jason_v1",val1:"$100",val2:"$250"},{user:"@emma_scale",val1:"$5K",val2:"$12.5K"}],a=i=>`
        ...
        </span>`;e.innerHTML="",t.forEach(i=>e.innerHTML+=a(i)),setInterval(()=>{e.style.transition="transform 500ms cubic-bezier(0.4, 0, 0.2, 1)",e.style.transform="translateY(-20px)",setTimeout(()=>{if(e.style.transition="none",e.appendChild(e.firstElementChild),e.style.transform="translateY(0)",Math.random()>.7){const s=Math.floor(Math.random()*50)*100,o=(1.5+Math.random()).toFixed(1),u={user:`@user_${Math.floor(Math.random()*900)+100}`,val1:`$${s}`,val2:`$${Math.floor(s*o)}`},x=document.createElement("div");x.innerHTML=a(u),e.appendChild(x.firstElementChild)}},500)},3e3),window.lucide&&window.lucide.createIcons()}function Z(){return`
        <style>
            @keyframes slideIn {
    ...
    `}function Q(){const e=["@jason_ship","@alice_dev","@sarah_builds","@founder_01","@yield_hunter","@anon_cap","@vector_vc","@protocol_labs","@sol_wizard"],t=[{type:"Contract Settled",status:"Settled",outcome:"Success",color:"emerald-700",flash:"flash-success",weight:4},{type:"Contract Forfeited",status:"Forfeited",outcome:"Failure",color:"[#921818]",flash:"flash-failure",weight:1},{type:"Contract Executed",status:"Locked",outcome:"—",color:"neutral-900",flash:"",weight:3},{type:"Identity Confirmed",status:"Confirmed",outcome:"—",color:"neutral-500",flash:"",weight:2}],a=["2.5x","1.8x","+$7,500","3.1x","+$12,400","1.5x","+$1,200","10.0x","+$500","4.2x"],i=[{ts:"14:02:11",type:"Contract Settled",hash:"0x8a7...2b91",user:"@jason_ship",val:"5,000.00",mult:"2.5x",status:"Settled",outcome:"Success",color:"emerald-700"},{ts:"14:02:05",type:"Identity Confirmed",hash:"0x8a7...2b91",user:"@jason_ship",val:"—",mult:"—",status:"Confirmed",outcome:"—",color:"neutral-500"},{ts:"14:01:44",type:"Contract Forfeited",hash:"0x3c2...9f44",user:"@demo_user",val:"1,200.00",mult:"—",status:"Forfeited",outcome:"Failure",color:"[#921818]"},{ts:"14:00:12",type:"Contract Executed",hash:"0xe11...5a02",user:"@alice_dev",val:"10,000.00",mult:"—",status:"Locked",outcome:"—",color:"neutral-900"},{ts:"13:58:30",type:"Contract Settled",hash:"0xf33...1b99",user:"@sarah_builds",val:"500.00",mult:"+$240",status:"Settled",outcome:"Success",color:"emerald-700"}],s=document.getElementById("ledger-body");if(!s)return;const o=15;function u(r,g){if(!g)return r[Math.floor(Math.random()*r.length)];const y=r.reduce((T,R)=>T+R.weight,0);let C=Math.random()*y;for(const T of r){if(C<T.weight)return T;C-=T.weight}return r[0]}function x(){return"0x"+Math.floor(Math.random()*16777215).toString(16)+"..."+Math.floor(Math.random()*65535).toString(16)}function f(){return(Math.random()*15e3+100).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}function B(){return new Date().toISOString().split("T")[1].split(".")[0]+" UTC"}function S(r,g=!1){const y=document.createElement("tr");y.className=`group hover:bg-neutral-50/50 transition-colors ${g?"animate-new-row "+(r.flash||""):""}`;const C=r.status==="Settled"?"text-emerald-700 font-semibold":"text-neutral-400";return y.innerHTML=`
        ...
        `,y}i.forEach(r=>{const g=t.find(y=>y.status===r.status);g&&(r.flash=g.flash),s.appendChild(S(r))});function L(){const r=u(t,"weight"),g=u(e),y=r.status==="Confirmed"?"—":f();let C="—";r.status==="Settled"&&(C=u(a));const T={ts:B(),type:r.type,hash:x(),user:g,val:y,mult:C,status:r.status,outcome:r.outcome,color:r.color,flash:r.flash},R=S(T,!0);for(s.firstChild?s.insertBefore(R,s.firstChild):s.appendChild(R);s.children.length>o;)s.removeChild(s.lastChild);const D=document.getElementById("global-tvl");if(D){let $=parseInt(D.innerText.replace(/,/g,"").replace(" USD",""));Math.random()>.5?$+=Math.floor(Math.random()*5e3):$-=Math.floor(Math.random()*2e3),D.innerText=$.toLocaleString()+" USD"}}function b(){const r=Math.random()*4e3+2e3;window.ledgerInterval=setTimeout(()=>{document.getElementById("ledger-body")&&(L(),b())},r)}b(),window.lucide&&window.lucide.createIcons()}function ee(){return`
        <style>
        ...
        </div>`}function te(){let e=1,t=[],a=null,i=null,s=null,o=!1,u=!1,x=null,f=null,B=!1,S=null;const L={TWITTER:{authority:"X",metricType:"FOLLOWERS",baselineValue:3842,currentValue:3842,goalValue:1e4,deadlineUtc:"2026-02-04T00:00:00Z",recordedAtUtc:"2026-01-04T19:43:00Z"},STRIPE:{authority:"STRIPE",metricType:"REVENUE",baselineValue:4224,currentValue:4224,goalValue:1e4,deadlineUtc:"2026-02-04T00:00:00Z",recordedAtUtc:"2026-01-04T19:43:00Z"}};async function b(n){return new Promise(l=>{setTimeout(()=>{l(L[i]||L.TWITTER)},100)})}function r(n){return n.toLocaleString("en-US")}function g(n){return"$"+n.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}function y(n){return new Date(n).toISOString().replace("T"," ").slice(0,19)+" UTC"}function C(n){const d=new Date(n)-new Date;return Math.max(0,Math.ceil(d/(1e3*60*60*24)))}function T(n){const l=n.metricType==="REVENUE",c=l?g:r,d=l?"":" followers",m=Math.max(0,n.goalValue-n.currentValue),A=n.goalValue>n.baselineValue?Math.min(1,Math.max(0,(n.currentValue-n.baselineValue)/(n.goalValue-n.baselineValue))):0,F=document.getElementById("metric-current"),I=document.getElementById("metric-goal"),E=document.getElementById("metric-recorded"),H=document.getElementById("metric-deadline"),z=document.getElementById("metric-required"),_=document.getElementById("metric-progress-fill"),j=document.getElementById("metric-authority-badge"),V=document.getElementById("metric-source-note");F&&(F.textContent=c(n.currentValue)+d),I&&(I.textContent=c(n.goalValue)+d),E&&(E.textContent="Recorded: "+y(n.recordedAtUtc)),H&&(H.textContent="Due in "+C(n.deadlineUtc)+" days"),z&&(z.textContent="+"+c(m)+d),_&&(_.style.width=A*100+"%"),n.authority==="X"?(j&&(j.textContent="X Verified"),V&&(V.textContent="Pulled from X (Twitter) API snapshots.")):n.authority==="STRIPE"&&(j&&(j.textContent="Stripe Verified"),V&&(V.textContent="Verified gross revenue via Stripe Connect."))}const R={TWITTER:{title:"Public Signal — X (Twitter)",integrity:"Integrity: High",measured:"Follower count / impressions (public).",verified:"Snapshots taken at execution + at deadline. Public API source-of-truth.",failcases:"Account private/suspended, API unavailable, or verification errors → fail closed."},STRIPE:{title:"Economic Signal — Stripe",integrity:"Integrity: Proven",measured:"Verified revenue (gross or net per contract definition).",verified:"Pulled from your connected Stripe account (proof-of-control required).",failcases:"Disconnected Stripe, missing permissions, Stripe outage → fail closed."}};function D(n,l=!1){const c=document.getElementById("progress-fill");if(c)if(l)c.style.width="100%";else{const d={1:"25%",2:"50%",3:"75%"};c.style.width=d[n]||"0%"}}function $(n){document.querySelectorAll(".step-item").forEach(d=>{const m=parseInt(d.dataset.step);d.classList.remove("active","completed","clickable"),m===n?d.classList.add("active"):t.includes(m)&&d.classList.add("completed","clickable")});const c=document.querySelector(".step-status");if(c){const d={1:"S: Profile",2:"S: Source",3:"S: Lock"};c.textContent=d[n]||"S: Ready"}}window.wizard={setStep:function(n){e=n,document.querySelectorAll('section[id^="step-"]').forEach(c=>c.classList.add("hidden"));const l=document.getElementById("step-"+n);l&&l.classList.remove("hidden"),$(n),D(n)},markCompleted:function(n){t.includes(n)||t.push(n),$(e)},goToStep:function(n){(t.includes(n)||n===e)&&this.setStep(n)},selectRisk:function(n,l){a=n,document.querySelectorAll("#step-1 button").forEach(c=>c.classList.remove("card-selected")),l.classList.add("card-selected"),document.getElementById("btn-step-1").disabled=!1},selectSource:function(n,l){i=n,document.querySelectorAll("#step-2 button").forEach(E=>E.classList.remove("card-selected")),l.classList.add("card-selected");const c=document.getElementById("authority-panel"),d=R[n];d&&(document.getElementById("panel-title").textContent=d.title,document.getElementById("panel-integrity").textContent=d.integrity,document.getElementById("panel-measured").textContent=d.measured,document.getElementById("panel-verified").textContent=d.verified,document.getElementById("panel-failcases").textContent=d.failcases,c.classList.add("visible"),document.getElementById("status-row").classList.add("hidden"));const m=document.getElementById("x-verify-panel"),A=document.getElementById("stripe-verify-panel");m.style.display="none",A.style.display="none",n==="TWITTER"?(m.style.display="block",document.getElementById("btn-step-2").disabled=!u):n==="STRIPE"?(A.style.display="block",document.getElementById("btn-step-2").disabled=!B):document.getElementById("btn-step-2").disabled=!1;const F=document.getElementById("metric-preview"),I=L[n];if(F&&I&&(n!=="TWITTER"||u)){F.style.display="block";const E=I.metricType==="REVENUE",H=E?g:r,z=E?"":" followers";document.getElementById("preview-current").textContent=H(I.currentValue)+z,document.getElementById("preview-authority-badge").textContent=n==="TWITTER"?"X":"Stripe"}window.lucide&&window.lucide.createIcons()},nextStep:function(){if(e===1)this.markCompleted(1),this.setStep(2);else if(e===2){this.markCompleted(2),this.setStep(3);let n="1.5x";a==="BOLD"&&(n="2.0x"),a==="ALL_IN"&&(n="4.0x"),document.getElementById("final-mult").textContent=n;const l={TWITTER:"X (Twitter)",STRIPE:"Stripe",GITHUB:"GitHub"};document.getElementById("final-oracle").textContent=l[i]||"Unknown",b().then(c=>{T(c)})}},startHold:function(){if(o)return;document.getElementById("btn-execute").classList.add("holding"),s=setTimeout(()=>{this.completeHold()},2e3)},endHold:function(){if(o)return;document.getElementById("btn-execute").classList.remove("holding"),clearTimeout(s)},completeHold:async function(){o=!0;const n=document.getElementById("btn-execute");document.getElementById("btn-text").textContent="Executing...",n.disabled=!0;try{const l=document.querySelector('#step-3 input[type="number"]'),c=parseFloat((l==null?void 0:l.value)||5e3),d=new Date;d.setDate(d.getDate()+30);const m={platform:i==="TWITTER"?"X":"STRIPE",metricType:i==="TWITTER"?"FOLLOWERS":"REVENUE",condition:{operator:"GTE",threshold:1e4,deadline:d.toISOString()},lockAmountUsdCents:Math.round(c*100),payoutAmountUsdCents:Math.round(c*100)},A=await window.api.createContract(m);console.log("[Contracts] Contract created:",A),this.markCompleted(3),document.getElementById("btn-text").textContent="Contract Executed",n.classList.remove("bg-neutral-900"),n.classList.add("bg-black","cursor-default");const F=document.querySelector(".step-status");F&&(F.textContent="S: Complete"),D(3,!0),setTimeout(()=>{var E;const I=A.contractId||((E=A.contract)==null?void 0:E.id);I?window.router.navigate("/contracts/"+I):window.router.navigate("/my-contracts")},1500)}catch(l){console.error("[Contracts] Create contract error:",l),o=!1,document.getElementById("btn-text").textContent="Hold to Execute",n.disabled=!1,alert("Contract creation failed: "+(l.message||"Unknown error"))}},generateVerifyCode:async function(){var d;const n=document.getElementById("x-username-input"),l=(d=n==null?void 0:n.value)==null?void 0:d.trim(),c=document.getElementById("x-generate-btn");if(!l){alert("Please enter your X username");return}c.disabled=!0,c.innerHTML='<div class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin mx-auto"></div>';try{const m=await window.api.startXVerification(l);if(console.log("[Contracts] startXVerification result:",m),x=l,f=m.challengeCode||m.code,!f&&m.codeMasked){alert("Backend update required. Please deploy the latest code to Railway and try again."),c.textContent="Generate Code",c.disabled=!1;return}if(!f){console.error("[Contracts] No code in response:",m),alert("Failed to get verification code. Please try again."),c.textContent="Generate Code",c.disabled=!1;return}document.getElementById("x-verify-step1").classList.add("hidden"),document.getElementById("x-verify-step2").classList.remove("hidden"),document.getElementById("x-verify-code").textContent=f,document.getElementById("x-verify-status").textContent="Pending",window.lucide&&window.lucide.createIcons()}catch(m){console.error("[Contracts] startXVerification error:",m),alert("Failed to generate code: "+(m.message||"Unknown error")),c.textContent="Generate Code",c.disabled=!1}},copyVerifyCode:function(){f&&navigator.clipboard.writeText(f).then(()=>{const n=document.querySelector('#x-verify-step2 button[onclick*="copyVerifyCode"]');if(n){const l=n.innerHTML;n.innerHTML='<i data-lucide="check" class="w-4 h-4"></i>',window.lucide&&window.lucide.createIcons(),setTimeout(()=>{n.innerHTML=l,window.lucide&&window.lucide.createIcons()},2e3)}})},verifyXAccount:async function(){const n=document.getElementById("x-verify-btn"),l=n.textContent;n.innerHTML='<div class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin mx-auto"></div>',n.disabled=!0;try{const c=await window.api.verifyX();console.log("[Contracts] X verification result:",c),u=!0,document.getElementById("x-verify-step1").classList.add("hidden"),document.getElementById("x-verify-step2").classList.add("hidden"),document.getElementById("x-verify-success").classList.remove("hidden"),document.getElementById("x-verified-handle").textContent="@"+x+" • Connected",document.getElementById("x-verify-status").textContent="Verified",document.getElementById("x-verify-status").classList.remove("text-neutral-400"),document.getElementById("x-verify-status").classList.add("text-[#1F7A4D]"),document.getElementById("btn-step-2").disabled=!1;const d=document.getElementById("metric-preview"),m=L.TWITTER;d&&m&&(d.style.display="block",document.getElementById("preview-current").textContent=r(m.currentValue)+" followers",document.getElementById("preview-authority-badge").textContent="X"),window.lucide&&window.lucide.createIcons()}catch(c){console.error("[Contracts] X verification error:",c),n.textContent=l,n.disabled=!1;const d=c.message||"Verification failed";d.includes("could not find")||d.includes("not found")?alert('Verification failed: Could not find the code "'+f+'" in your X bio. Please add it and try again.'):alert("Verification failed: "+d)}},startStripeConnect:async function(){const n=document.getElementById("stripe-connect-btn");n.disabled=!0,n.innerHTML='<div class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin mx-auto"></div>';try{const l=await window.api.startStripeConnect();if(console.log("[Contracts] startStripeConnect result:",l),l.alreadyConnected){B=!0,S=l.stripeAccountId,document.getElementById("stripe-connect-step1").classList.add("hidden"),document.getElementById("stripe-connect-success").classList.remove("hidden"),document.getElementById("stripe-account-id").textContent=S+" • Connected",document.getElementById("stripe-verify-status").textContent="Connected",document.getElementById("stripe-verify-status").classList.remove("text-neutral-400"),document.getElementById("stripe-verify-status").classList.add("text-[#1F7A4D]"),document.getElementById("btn-step-2").disabled=!1,window.lucide&&window.lucide.createIcons();return}localStorage.setItem("stripe_oauth_state",l.state);const c=window.open(l.oauthUrl,"stripe-connect","width=600,height=700,scrollbars=yes"),d=setInterval(async()=>{if(c.closed){clearInterval(d),n.textContent="Connect with Stripe",n.disabled=!1;const m=await window.api.getStripeStatus();m.connected&&(B=!0,S=m.stripeAccountId,document.getElementById("stripe-connect-step1").classList.add("hidden"),document.getElementById("stripe-connect-success").classList.remove("hidden"),document.getElementById("stripe-account-id").textContent=S+" • Connected",document.getElementById("stripe-verify-status").textContent="Connected",document.getElementById("stripe-verify-status").classList.remove("text-neutral-400"),document.getElementById("stripe-verify-status").classList.add("text-[#1F7A4D]"),document.getElementById("btn-step-2").disabled=!1,window.lucide&&window.lucide.createIcons())}},500)}catch(l){console.error("[Contracts] startStripeConnect error:",l),n.innerHTML='<svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M13.976 9.15c-2.172-.806-3.356-1.426-3.356-2.409 0-.831.683-1.305 1.901-1.305 2.227 0 4.515.858 6.09 1.631l.89-5.494C18.252.975 15.697 0 12.165 0 9.667 0 7.589.654 6.104 1.872 4.56 3.147 3.757 4.992 3.757 7.218c0 4.039 2.467 5.76 6.476 7.219 2.585.92 3.445 1.574 3.445 2.583 0 .98-.84 1.545-2.354 1.545-1.875 0-4.965-.921-6.99-2.109l-.9 5.555C5.175 22.99 8.385 24 11.714 24c2.641 0 4.843-.624 6.328-1.813 1.664-1.305 2.525-3.236 2.525-5.732 0-4.128-2.524-5.851-6.591-7.305z"/></svg> Connect with Stripe',n.disabled=!1,alert("Failed to start Stripe Connect: "+(l.message||"Unknown error"))}}},D(1)}function se(e){const t=(e==null?void 0:e.id)||"4421";return`
        ...
        </div>`}function ne(){window.lucide&&window.lucide.createIcons()}function ae(){return`
        <div class="pb-32 w-full max-w-5xl mx-auto px-6 relative z-10 min-h-screen">
            <!-- Breadcrumb -->
    ...
    `}function oe(){const e=document.querySelectorAll(".profile-tab"),t=document.querySelectorAll(".tab-panel");e.forEach(a=>{a.addEventListener("click",()=>{const i=a.getAttribute("data-tab");e.forEach(o=>{o.classList.remove("border-neutral-900","text-neutral-900","font-medium"),o.classList.add("border-transparent","text-neutral-400")}),a.classList.remove("border-transparent","text-neutral-400"),a.classList.add("border-neutral-900","text-neutral-900","font-medium"),t.forEach(o=>{o.classList.add("hidden")});const s=document.getElementById("tab-"+i);s&&s.classList.remove("hidden"),window.lucide&&window.lucide.createIcons()})}),window.lucide&&window.lucide.createIcons()}function ie(){return`
        <div class="pb-32 w-full max-w-5xl mx-auto px-6 relative z-10 min-h-screen">
            <!-- Breadcrumb -->
            <div class="flex items-center gap-2 mb-4 font-mono text-[10px] text-neutral-400 uppercase tracking-widest mt-8">
                <span>Collateral</span>
    ...
    `}function re(){const e=document.querySelectorAll(".contracts-tab"),t=document.querySelectorAll(".contracts-panel");e.forEach(a=>{a.addEventListener("click",()=>{const i=a.getAttribute("data-tab");e.forEach(o=>{o.classList.remove("border-neutral-900","text-neutral-900","font-medium"),o.classList.add("border-transparent","text-neutral-400")}),a.classList.remove("border-transparent","text-neutral-400"),a.classList.add("border-neutral-900","text-neutral-900","font-medium"),t.forEach(o=>o.classList.add("hidden"));const s=document.getElementById("contracts-tab-"+i);s&&s.classList.remove("hidden"),window.lucide&&window.lucide.createIcons()})}),window.lucide&&window.lucide.createIcons()}function le(){return`
        <div class="pb-32 w-full max-w-5xl mx-auto px-6 relative z-10 min-h-screen">
            <!-- Page Header -->
    ...
    `}function pe(){window.lucide&&window.lucide.createIcons()}function ue(){return`
        <style>
            .receipts-page {
                max-width: 900px;
                margin: 0 auto;
    ...
    `}function me(){const e=document.getElementById("receipts-content"),t=[];if(t.length===0){e.innerHTML=`
    ...
    `;t.forEach(i=>{const s=i.status==="SETTLED_SUCCESS"?"success":i.status==="SETTLED_FAILURE"?"failure":"active",o=i.status==="SETTLED_SUCCESS"?"Settled":i.status==="SETTLED_FAILURE"?"Forfeited":"Active";a+=`
        ...
        `}),a+="</div>",e.innerHTML=a}function xe(){return`
        <style>
            /* Receipt Page Styles */
            .receipt-page {
                max-width: 900px;
    ...
    `}function fe(e){const t=document.getElementById("receipt-container"),a=e==null?void 0:e.id,s={"RCP-001":{receiptId:"RCP-0x8a72390f1d2b4c5e6f7a8b9cd",contractId:"CTR-0x7A3F9E2B1C4D5E6F7A8B9CD4D1",executionTimestamp:"2026-01-04T19:43:22Z",status:"ACTIVE",terms:{authority:"X (Twitter)",metric:"Followers",baseline:3842,target:1e4,timeWindow:30,capitalLocked:5e3,multiplier:2,failureCondition:"Capital forfeiture"},snapshot:{source:"X API v2",value:3842,unit:"followers",capturedAt:"2026-01-04T19:43:22Z"},ledger:{eventId:"EVT-0x9f3d2a1b4c5e6f7e8",eventType:"CONTRACT_EXECUTED",hashChain:"0xa8b3c9d4e5f67890abcdef12345672a1",prevHash:"0x7c2d1e0f8a9b3c4d5e6f70819b3",appendTimestamp:"2026-01-04T19:43:22.847Z"},deadline:"2026-02-03T19:43:22Z"}}[a];if(!s){t.innerHTML=`
            <div class="receipt-not-found">
                <h1>Record Not Found</h1>
                <p>The requested receipt does not exist or is not accessible.</p>
            </div>
        `;return}function o(b){return b.toLocaleString("en-US")}function u(b){return"$"+b.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}function x(b){return new Date(b).toISOString().replace("T"," ").slice(0,19)+" UTC"}function f(b,r=12,g=3){return b.length<=r+g+3?b:b.slice(0,r)+"..."+b.slice(-g)}const B=s.status==="SETTLED_SUCCESS"?"success":s.status==="SETTLED_FAILURE"?"failure":"active",S=s.status==="SETTLED_SUCCESS"?"SETTLED — SUCCESS":s.status==="SETTLED_FAILURE"?"SETTLED — FAILURE":"ACTIVE",L=s.status==="ACTIVE"?"Settlement will occur automatically at deadline.":s.status==="SETTLED_SUCCESS"?`Target achieved. Payout: ${u(s.terms.capitalLocked*s.terms.multiplier)}`:`Target not achieved. Capital forfeited: ${u(s.terms.capitalLocked)}`;t.innerHTML=`
        <header class="receipt-header">
            ...
            <div class="receipt-meta">
                <div class="receipt-meta-item">
                    ...
                    <span class="receipt-meta-value hash">${f(s.receiptId)}</span>
                </div>
                <div class="receipt-meta-item">
                    <span class="receipt-meta-label">Contract ID</span>
                    <span class="receipt-meta-value hash">${f(s.contractId)}</span>
                </div>
                <div class="receipt-meta-item">
                    <span class="receipt-meta-label">Execution Timestamp</span>
                    <span class="receipt-meta-value">${x(s.executionTimestamp)}</span>
                </div>
                <div class="receipt-meta-item">
        ...
        <section class="receipt-section">
            ...
            <div class="snapshot-box">
                ...
                <div class="snapshot-timestamp">Captured: ${x(s.snapshot.capturedAt)}</div>
        ...
        <section class="receipt-section">
            ...
            <div class="ledger-box">
                <div class="ledger-row">
                    ...
                    <span class="ledger-value">${f(s.ledger.eventId)}</span>
                </div>
                <div class="ledger-row">
                ...
                <div class="ledger-row">
                    ...
                    <span class="ledger-value">${f(s.ledger.hashChain)}</span>
                </div>
                <div class="ledger-row">
                    <span class="ledger-label">Previous Event Hash</span>
                    <span class="ledger-value">${f(s.ledger.prevHash)}</span>
                </div>
                <div class="ledger-row">
                    <span class="ledger-label">Append Timestamp</span>
                    <span class="ledger-value">${x(s.ledger.appendTimestamp)}</span>
                </div>
        ...
        <section class="receipt-section">
            ...
            <div class="status-block ${B}">
                <div>
                    <div class="status-label ${B}">${S}</div>
                    <div class="status-detail">${L}</div>
                </div>
                <div class="status-timestamp">Deadline: ${x(s.deadline)}</div>
            </div>
        </section>
    ...
    `}const v="https://collateral-production.up.railway.app",U="collateral_auth_token",N="collateral_user";function P(){return localStorage.getItem(U)}function be(e){localStorage.setItem(U,e)}function G(){localStorage.removeItem(U),localStorage.removeItem(N)}function ge(){return!!P()}function ve(){const e=localStorage.getItem(N);return e?JSON.parse(e):null}function he(e){localStorage.setItem(N,JSON.stringify(e))}function w(e=!0){const t={"Content-Type":"application/json"};if(e){const a=P();a&&(t.Authorization=`Bearer ${a}`)}return t}async function h(e){const t=await e.json().catch(()=>({}));if(!e.ok){const a=new Error(t.message||t.error||`HTTP ${e.status}`);throw a.status=e.status,a.code=t.code,a.data=t,a}return t}async function we(e){var i;console.log("[API] devLogin called with email:",e);const t=await fetch(`${v}/auth/login`,{method:"POST",headers:w(!1),body:JSON.stringify({email:e})}),a=await h(t);return console.log("[API] devLogin response:",a),console.log("[API] accessToken in response:",!!a.accessToken),a.accessToken?(be(a.accessToken),he({email:e,userId:(i=a.user)==null?void 0:i.id}),console.log("[API] Token stored! Verifying:",!!P())):console.warn("[API] No accessToken in response!"),a}async function ye(){G()}async function Ee(e){console.log("[API] startXVerification called with username:",e);const t=await fetch(`${v}/v1/connect/x/start`,{method:"POST",headers:w(),body:JSON.stringify({username:e})});return h(t)}async function ke(){const e=w();console.log("[API] verifyX called"),console.log("[API] Auth token present:",!!P());const t=await fetch(`${v}/v1/connect/x/verify`,{method:"POST",headers:e,body:JSON.stringify({})});return h(t)}async function Se(){const e=await fetch(`${v}/v1/connect/x/status`,{method:"GET",headers:w()});return h(e)}async function Ce(){console.log("[API] startStripeConnect called");const e=await fetch(`${v}/v1/connect/stripe/start`,{method:"GET",headers:w()});return h(e)}async function Te(e,t){console.log("[API] completeStripeConnect called");const a=await fetch(`${v}/v1/connect/stripe/callback`,{method:"POST",headers:w(),body:JSON.stringify({code:e,state:t})});return h(a)}async function Ie(){const e=await fetch(`${v}/v1/connect/stripe/status`,{method:"GET",headers:w()});return h(e)}async function Be(e){const t=await fetch(`${v}/v1/contracts`,{method:"POST",headers:w(),body:JSON.stringify(e)});return h(t)}async function Le(){const e=await fetch(`${v}/v1/contracts`,{method:"GET",headers:w()});return h(e)}async function Ae(e){const t=await fetch(`${v}/v1/contracts/${e}`,{method:"GET",headers:w()});return h(t)}async function Fe(e){const t=await fetch(`${v}/v1/contracts/${e}/fund`,{method:"POST",headers:w()});return h(t)}async function De(e){const t=await fetch(`${v}/v1/contracts/${e}/events`,{method:"GET",headers:w()});return h(t)}async function Me(){const e=await fetch(`${v}/v1/ledger`,{method:"GET",headers:w(!1)});return h(e)}async function Re(){const e=await fetch(`${v}/health`,{method:"GET"});return h(e)}const M={devLogin:we,logout:ye,getAuthToken:P,hasAuthToken:ge,clearAuthToken:G,getStoredUser:ve,startXVerification:Ee,verifyX:ke,getXStatus:Se,startStripeConnect:Ce,completeStripeConnect:Te,getStripeStatus:Ie,createContract:Be,getContracts:Le,getContract:Ae,createFundingIntent:Fe,getLedgerEvents:De,getPublicLedger:Me,checkHealth:Re},k=M.getStoredUser();var X;const p={isLoggedIn:M.hasAuthToken(),username:k!=null&&k.email?"@"+k.email.split("@")[0]:null,displayName:((X=k==null?void 0:k.email)==null?void 0:X.split("@")[0])||null,userId:(k==null?void 0:k.userId)||null,connectedSources:{twitter:!1,github:!1,stripe:!1}};window.appState=p;window.api=M;const $e=[{path:"/overview",render:J,init:Y},{path:"/ledger",render:Z,init:Q},{path:"/contracts",render:ee,init:te},{path:"/contracts/:id",render:se,init:ne},{path:"/profile",render:ae,init:oe},{path:"/my-contracts",render:ie,init:re},{path:"/docs",render:le,init:ce},{path:"/funding",render:de,init:pe},{path:"/receipts",render:ue,init:me},{path:"/receipts/:id",render:xe,init:fe}],W=new q($e);window.router=W;window.app={openAccessModal:function(){const e=document.getElementById("modal-access-backdrop"),t=document.getElementById("modal-access");e.classList.remove("hidden"),t.classList.remove("hidden"),setTimeout(()=>{e.classList.remove("opacity-0"),t.classList.remove("scale-95","opacity-0"),t.classList.add("scale-100","opacity-100")},10)},closeAccessModal:function(){const e=document.getElementById("modal-access-backdrop"),t=document.getElementById("modal-access");e.classList.add("opacity-0"),t.classList.add("scale-95","opacity-0"),t.classList.remove("scale-100","opacity-100"),setTimeout(()=>{e.classList.add("hidden"),t.classList.add("hidden")},300)},handleAuthClick:function(){p.isLoggedIn?window.router.navigate("/profile"):window.app.openAccessModal()},handleInitiate:function(){p.isLoggedIn?window.router.navigate("/contracts"):window.app.openAccessModal()},handleLoginSubmit:async function(){const e=document.getElementById("btn-login-submit"),t=document.getElementById("login-email"),a=(t==null?void 0:t.value)||"demo@collateral.market",i=e.innerText;e.innerHTML='<div class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin mx-auto"></div>',e.disabled=!0;try{const s=await M.devLogin(a);p.isLoggedIn=!0,p.username="@"+a.split("@")[0],p.userId=s.userId,window.app.closeAccessModal(),O()}catch(s){console.error("Login failed:",s),alert("Login failed: "+(s.message||"Unknown error"))}finally{e.innerText=i,e.disabled=!1}},goToCreateIdentity:function(){window.app.closeAccessModal(),setTimeout(()=>{window.app.openCreateModal()},350)},openCreateModal:function(){const e=document.getElementById("modal-create-backdrop"),t=document.getElementById("modal-create");e.classList.remove("hidden"),t.classList.remove("hidden"),setTimeout(()=>{e.classList.remove("opacity-0"),t.classList.remove("scale-95","opacity-0"),t.classList.add("scale-100","opacity-100")},10)},closeCreateModal:function(){const e=document.getElementById("modal-create-backdrop"),t=document.getElementById("modal-create");e.classList.add("opacity-0"),t.classList.add("scale-95","opacity-0"),t.classList.remove("scale-100","opacity-100"),setTimeout(()=>{e.classList.add("hidden"),t.classList.add("hidden")},300)},handleCreateAccount:async function(){const e=document.getElementById("btn-create-submit"),t=document.getElementById("create-email"),a=document.getElementById("create-displayname").value,i=(t==null?void 0:t.value)||a+"@collateral.market",s=e.innerText;e.innerHTML='<div class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin mx-auto"></div>',e.disabled=!0;try{console.log("[App] Creating account with email:",i);const o=await M.devLogin(i);console.log("[App] Account created, token stored:",!!M.getAuthToken()),p.isLoggedIn=!0,p.username="@"+(a||i.split("@")[0]),p.userId=o.userId,window.app.closeCreateModal(),O()}catch(o){console.error("Account creation failed:",o),alert("Account creation failed: "+(o.message||"Unknown error"))}finally{e.innerText=s,e.disabled=!1}},handleSignOut:function(){M.logout(),p.isLoggedIn=!1,p.username=null,p.userId=null,O(),window.router.navigate("/overview")},toggleMenuPersistence:function(e){e.stopPropagation();const t=document.getElementById("user-dropdown-content");t&&t.classList.toggle("!block")},connectSource:function(e){const t=document.getElementById(e+"-btn"),a=document.getElementById(e+"-status");document.getElementById("source-"+e),t&&(t.innerText,t.innerHTML='<div class="w-3 h-3 border-2 border-neutral-300 border-t-neutral-600 rounded-full animate-spin"></div>',t.disabled=!0,setTimeout(()=>{var o,u;p.connectedSources[e]=!0;const i={twitter:"@"+(((o=p.username)==null?void 0:o.replace("@",""))||"user"),github:((u=p.username)==null?void 0:u.replace("@",""))||"user",stripe:"acct_"+Math.random().toString(36).substr(2,6)};if(a.textContent=i[e]+" • Connected",a.classList.remove("text-neutral-400"),a.classList.add("text-neutral-500"),t.outerHTML=`
                <span class="flex items-center gap-1.5 text-[#1F7A4D] text-[11px] font-mono uppercase">
                    <i data-lucide="check-circle" class="w-4 h-4"></i>
                    Verified
                </span>
            `,window.lucide&&window.lucide.createIcons(),Object.values(p.connectedSources).some(x=>x)){const x=document.getElementById("sources-alert");x&&x.classList.add("hidden")}},1500))},openCardModal:function(){const e=document.getElementById("modal-card-backdrop"),t=document.getElementById("modal-card");e.classList.remove("hidden"),t.classList.remove("hidden"),setTimeout(()=>{e.classList.remove("opacity-0"),t.classList.remove("scale-95","opacity-0"),t.classList.add("scale-100","opacity-100")},10),window.lucide&&window.lucide.createIcons()},closeCardModal:function(){const e=document.getElementById("modal-card-backdrop"),t=document.getElementById("modal-card");e.classList.add("opacity-0"),t.classList.add("scale-95","opacity-0"),t.classList.remove("scale-100","opacity-100"),setTimeout(()=>{e.classList.add("hidden"),t.classList.add("hidden"),document.getElementById("card-number").value="",document.getElementById("card-expiry").value="",document.getElementById("card-cvc").value=""},300)},addCard:function(){window.app.openCardModal()},confirmCardSetup:function(){const e=document.getElementById("btn-card-submit"),t=document.getElementById("card-number").value,a=document.getElementById("card-expiry").value,i=document.getElementById("card-cvc").value;if(!t||!a||!i)return;const s=e.innerHTML;e.innerHTML='<div class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>',e.disabled=!0,setTimeout(()=>{const o=t.replace(/\s/g,"").slice(-4),u=document.getElementById("card-status"),x=document.getElementById("add-card-btn");u&&(u.textContent=`•••• •••• •••• ${o} • Active`,u.classList.remove("text-neutral-400"),u.classList.add("text-neutral-500")),x&&(x.innerHTML="Remove",x.onclick=()=>window.app.removeCard()),p.hasCard=!0,p.cardLast4=o,window.app.closeCardModal(),e.innerHTML=s,e.disabled=!1},1500)},removeCard:function(){const e=document.getElementById("add-card-btn"),t=document.getElementById("card-status");t.textContent="No card on file",t.classList.remove("text-neutral-500"),t.classList.add("text-neutral-400"),e.innerHTML="Add Card",e.onclick=()=>window.app.addCard(),p.hasCard=!1,p.cardLast4=null,e.innerHTML="Add Card",e.onclick=()=>window.app.addCard()},setupPayout:function(){const e=document.getElementById("setup-payout-btn"),t=document.getElementById("payout-status");e&&(e.innerHTML='<div class="w-3 h-3 border-2 border-neutral-300 border-t-neutral-600 rounded-full animate-spin"></div>',e.disabled=!0,setTimeout(()=>{t.textContent="Bank account ••••6789 • Active",t.classList.remove("text-neutral-400"),t.classList.add("text-neutral-500"),e.innerHTML="Update",e.disabled=!1},2e3))},openSettingsModal:function(){const e=document.getElementById("modal-settings-backdrop"),t=document.getElementById("modal-settings");e.classList.remove("hidden"),t.classList.remove("hidden"),setTimeout(()=>{e.classList.remove("opacity-0"),t.classList.remove("scale-95","opacity-0"),t.classList.add("scale-100","opacity-100")},10);const a=document.getElementById("settings-username");a&&p.username&&(a.textContent=p.username),window.app.populateSettingsSources(),window.lucide&&window.lucide.createIcons(),window.app.initSettingsTabs()},closeSettingsModal:function(){const e=document.getElementById("modal-settings-backdrop"),t=document.getElementById("modal-settings");e.classList.add("opacity-0"),t.classList.add("scale-95","opacity-0"),t.classList.remove("scale-100","opacity-100"),setTimeout(()=>{e.classList.add("hidden"),t.classList.add("hidden"),window.app.switchSettingsTab("account")},300)},initSettingsTabs:function(){document.querySelectorAll(".settings-tab").forEach(t=>{t.onclick=()=>{const a=t.getAttribute("data-settings-tab");window.app.switchSettingsTab(a)}})},switchSettingsTab:function(e){const t=document.querySelectorAll(".settings-tab"),a=document.querySelectorAll(".settings-panel");t.forEach(o=>{o.classList.remove("bg-neutral-100","bg-red-50","text-neutral-900"),o.classList.add("text-neutral-500"),o.getAttribute("data-settings-tab")==="danger"&&(o.classList.add("text-[#B91C1C]"),o.classList.remove("text-neutral-500"))});const i=document.querySelector(`[data-settings-tab="${e}"]`);i&&(e==="danger"?i.classList.add("bg-red-50"):(i.classList.add("bg-neutral-100","text-neutral-900"),i.classList.remove("text-neutral-500"))),a.forEach(o=>o.classList.add("hidden"));const s=document.getElementById("settings-panel-"+e);s&&s.classList.remove("hidden"),window.lucide&&window.lucide.createIcons()},populateSettingsSources:function(){var a,i,s;const e=document.getElementById("settings-sources-list");if(!e)return;const t=[{id:"stripe",name:"Stripe",icon:"credit-card",connected:(a=p.connectedSources)==null?void 0:a.stripe},{id:"github",name:"GitHub",icon:"github",connected:(i=p.connectedSources)==null?void 0:i.github},{id:"twitter",name:"X (Twitter)",icon:"twitter",connected:(s=p.connectedSources)==null?void 0:s.twitter}];e.innerHTML=t.map(o=>`
        ...
        `).join(""),window.lucide&&window.lucide.createIcons()},disconnectSource:function(e){p.connectedSources[e]=!1,window.app.populateSettingsSources()}};function O(){const e=document.getElementById("btn-auth"),t=document.getElementById("user-menu");p.isLoggedIn&&e&&t?(e.classList.add("hidden"),t.classList.remove("hidden"),document.getElementById("menu-username").innerText=p.username):e&&t&&(e.classList.remove("hidden"),t.classList.add("hidden"))}const Pe=["/contracts","/my-contracts","/profile","/funding"];W.onRouteChange=function(e,t){if(Pe.some(o=>t===o||t.startsWith(o+"/"))&&!p.isLoggedIn){window.app.openAccessModal(),window.location.hash="/overview";return}const i=document.getElementById("header-mount");i.innerHTML=K(t);const s=document.getElementById("app");s.innerHTML=e.render(e.params),e.init&&setTimeout(()=>e.init(),0),window.lucide&&setTimeout(()=>window.lucide.createIcons(),10),O()};window.location.hash||(window.location.hash="/overview");
