...
.lbtn-r{background:var(--r) !important;color:#fff !important;box-shadow:0 2px 8px rgba(92,20,20,.15)}.lbtn-r:hover{background:var(--rh) !important;box-shadow:0 4px 16px rgba(92,20,20,.2);transform:translateY(-1px) scale(1.02)}
...
/* ═══ SOCIAL PROOF STATS ═══ */
.lstats{background:var(--t1);padding:0;border-bottom:1px solid rgba(255,255,255,.06)}
.lstats-inner{max-width:1080px;margin:0 auto;display:flex;align-items:center;justify-content:center}
.lstat{flex:1;text-align:center;padding:24px 16px}
.lstat-val{font-family:'SF Mono','Fira Code','Consolas',monospace;font-size:20px;font-weight:800;color:#fff;letter-spacing:-.5px;margin-bottom:4px}
.lstat-label{font-family:'Inter',monospace;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1.5px;color:rgba(255,255,255,.4)}
.lstat-sep{width:1px;height:40px;background:rgba(255,255,255,.08);flex-shrink:0}
...
.lcard{border:1px solid var(--d);padding:36px 32px;display:flex;flex-direction:column;transition:border-color .15s,box-shadow .15s,transform .2s;background:var(--p)}
...
.lcard-btn button{width:100%;height:44px;background:var(--t1);color:#fff;font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;border:none;cursor:pointer;transition:background .15s,transform .15s}
