 ...
 */
export function isQualifiedRevenueCharge(charge, expectedCurrency) {
    // Rule 1: Must be paid
    if (!charge.paid) {
 ...
 */
export function validateTierEligibility(baselineNetRevenueCents, tier) {
 ...
 */
export function checkSingleInvoiceDominance(charges) {
    if (charges.length === 0) {
        return {
            violation: false,
 ...
 */
export function calculateStripeDeltaFloor(baselineNetRevenueCents, tierPercentage = STRIPE_DELTA_FLOOR_PERCENTAGE) {
 ...
 */
export function validateStripeDeltaFloor(baselineNetRevenueCents, deltaTargetCents, tierPercentage = STRIPE_DELTA_FLOOR_PERCENTAGE) {
 ...
 */
export class RealStripeRevenueClient {
    stripe;
    maxRetries = 3;
    retryDelayMs = 1000;
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (!isRetryable) {
                    ...
                    throw new Error(`[${STRIPE_ERROR_CODES.API_UNAVAILABLE}] ` +
    ...
    async getRevenueInWindow(connectedAccountId, start, end) {
        const result = await this.getNetSettledRevenue(connectedAccountId, start, end, end, // No refund buffer for simple window query
        ...
        return this.withRetry(async () => {
            ...
            while (hasMore) {
                ...
                for (const txn of response.data) {
                    if (STRIPE_EXCLUDED_TRANSACTION_TYPES.includes(txn.type))
                        continue;
                    if (txn.currency.toLowerCase() !== STRIPE_ALLOWED_CURRENCY)
                        continue;
                    if (STRIPE_ALLOWED_TRANSACTION_TYPES.includes(txn.type)) {
                        total += txn.amount;
                    }
...
export const stripeRevenueAdapter = {
     ...
     */
    async createV1BaselineSnapshot(stripeConnectedAccountId, deltaTargetCents, executionTime, tier = 'STEADY') {
        const client = getRevenueClient();
        ...
        if (!tierEligibility.eligible) {
            throw new Error(`[${STRIPE_ERROR_CODES.INELIGIBLE_BASELINE_TOO_LOW}] ` +
        ...
        if (!deltaValidation.valid) {
            throw new Error(`[${STRIPE_ERROR_CODES.DELTA_FLOOR_NOT_MET}] ` +
