...
export function getXOAuthConfig() {
    if (!X_CLIENT_ID || !X_CLIENT_SECRET || !X_REDIRECT_URI) {
        throw new XAdapterError('X OAuth not configured. Set X_CLIENT_ID, X_CLIENT_SECRET, and X_REDIRECT_URI.', false, 'CONFIG');
    }
    return {
 ...
 */
export function getXAuthUrl(state, codeChallenge) {
...
export async function exchangeCodeForToken(code, codeVerifier) {
    ...
    if (!response.ok) {
        ...
        throw new XAdapterError(`X token exchange failed (${response.status}): ${body}`, false, 'AUTH');
    }
    ...
    if (!userResponse.ok) {
        throw new XAdapterError(`Failed to get X user info (${userResponse.status})`, false, 'AUTH');
    }
...
export async function refreshAccessToken(refreshToken) {
    ...
    if (!response.ok) {
        const body = await response.text();
        throw new XAdapterError(`X token refresh failed (${response.status}): ${body}`, false, 'AUTH');
    }
    const tokenData = await response.json();
    return {
        accessToken: tokenData.access_token,
