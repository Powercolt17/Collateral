 ...
 */
export function classifyAdapterError(err) {
    if (!err) {
        return {
            retryable: true,
 ...
 */
function calculateBackoff(attempt, baseMs = 5000, maxMs = 300000 // 5 minutes
) {
...
export async function scheduleRetry(params) {
    const { contractId, jobType, reason, attempt, baseBackoffMs = 5000, maxBackoffMs = 300000, } = params;
    ...
    console.log(`🔄 Retry scheduled: ${jobType} for ${contractId.slice(0, 8)}... ` +
 ...
 */
export async function clearRetrySchedule(contractId) {
    await db
        .update(contractIndex)
        .set({
 ...
 */
export async function runWithFailClosed(contractId, jobType, attempt, fn) {
    try {
...
//# sourceMappingURL=adapter-errors.js.map