...
const contractReadRoutes = async (fastify) => {
    /**
     * GET /v1/contracts
    ...
    fastify.get('/v1/contracts', async (request, reply) => {
        ...
        contractsWithState.sort((a, b) => b.contract.createdAt.getTime() - a.contract.createdAt.getTime());
        return {
     ...
     */
    fastify.get('/v1/contracts/:id', async (request, reply) => {
     ...
     */
    fastify.get('/v1/contracts/:id/events', async (request, reply) => {
        const { id } = request.params;
        const userId = request.userId;
        if (!userId) {
            reply.status(401);
...
//# sourceMappingURL=contracts-read.js.map