...
const contractWriteRoutes = async (fastify) => {
     ...
     * To-state: CREATED
     */
    fastify.post('/v1/contracts', async (request, reply) => {
     ...
     */
    fastify.post('/v1/contracts/:id/funding-intent', async (request, reply) => {
     ...
     */
    fastify.post('/v1/contracts/:id/execute', async (request, reply) => {
        const { id } = request.params;
        const principalUserId = getPrincipal(request);
        // Reject userId in body (spoof protection)
        if (request.body?.userId !== undefined) {
        ...
        try {
            ...
            if (contract.platform === 'X') {
                ...
                const [xAccount] = await db
                    ...
                    .where(and(eq(connectedAccounts.userId, principalUserId), eq(connectedAccounts.platform, 'X'), eq(connectedAccounts.status, 'ACTIVE'), eq(connectedAccounts.verificationStatus, 'VERIFIED')))
                    .limit(1);
                if (!xAccount) {
                    reply.status(400);
                    return {
                ...
                const xUsername = xAccount.metadataJson?.resolvedUsername || xAccount.externalAccountId;
...
//# sourceMappingURL=contracts-write.js.map