...
const webhookRoutes = async (fastify) => {
     ...
     */
    fastify.post('/v1/stripe/webhook', {
        config: {
        ...
        if (signature) {
            ...
            try {
                ...
                event = stripeClient.webhooks.constructEvent(rawBody, // Pass Buffer directly, not toString()
            ...
            if (!chargeId) {
                ...
                try {
                    ...
                    if (stripeClient && piId) {
                        ...
                        const fetchedChargeId = pi?.latest_charge?.id ?? pi?.latest_charge;
                        if (fetchedChargeId) {
