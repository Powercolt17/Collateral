 ...
 */
export async function handlePaymentSuccess(paymentIntentId, contractId, chargeId // REQUIRED: For dispute correlation
) {
    ...
    const fundingIntentExists = await eventExistsForExternalRef(contractId, paymentIntentId, EventType.FUNDS_AUTHORIZED);
    if (!fundingIntentExists) {
        throw new Error(`Untrusted payment binding: No funding intent found for payment_intent ${paymentIntentId} on contract ${contractId}`);
    }
...
export async function handlePaymentDisputed(params) {
    const { disputeId, chargeId, contractId, reason, amountCents, status } = params;
    // 1. IDEMPOTENCY CHECK
    const alreadyProcessed = await eventExistsForExternalRef(contractId, disputeId, EventType.PAYMENT_DISPUTED);
    if (alreadyProcessed) {
    ...
    if (currentState === ContractStatus.LOCKED ||
        ...
        await appendEvent({
            ...
            metadata: {
                ...
                auditTrail: {
                    termsHash: contract.baselineJson?.termsHash,
                    baselineWindow: contract.baselineJson?.baselineWindow,
                    verificationWindow: contract.baselineJson?.verificationWindow,
                    noAppeals: true,
