 ...
 */
export async function hasActiveLock(contractId, jobType) {
    const now = new Date();
    const [existingLock] = await db
        .select()
        .from(jobLocks)
        .where(and(eq(jobLocks.contractId, contractId), eq(jobLocks.jobType, jobType)))
        .limit(1);
    if (!existingLock) {
        return { locked: false };
    }
 ...
 */
export async function tryAcquireLock(contractId, jobType) {
    const now = new Date();
    ...
    try {
        ...
        await db
            .delete(jobLocks)
            .where(and(eq(jobLocks.contractId, contractId), eq(jobLocks.jobType, jobType), lt(jobLocks.expiresAtUtc, now)));
 ...
 */
export async function releaseLock(contractId, jobType, lockId) {
    const result = await db
        .delete(jobLocks)
        .where(and(eq(jobLocks.contractId, contractId), eq(jobLocks.jobType, jobType), eq(jobLocks.lockId, lockId)))
        .returning();
 ...
 */
export async function getRetryCount(contractId, jobType) {
    const events = await getEventsForContract(contractId);
    return events.filter(e => {
        if (e.eventType !== EventType.RETRY_SCHEDULED)
            return false;
 ...
 */
export async function scheduleRetry(contractId, jobType, reason, baseDelayMs = 60000 // 1 minute base
) {
 ...
 */
export async function getNextRetryTime(contractId, jobType) {
    const events = await getEventsForContract(contractId);
