...
import { users, ContractStatus, EventType, contractIndex } from '../db/schema.js';
...
export async function issueReceiptForContract(contractId, contract, terminalEvent, tx) {
    ...
    const verificationEvent = latestEvents.find(e => e.eventType === EventType.VERIFICATION_SUCCEEDED ||
        e.eventType === EventType.VERIFICATION_FAILED);
    const verificationEvidence = verificationEvent?.metadataJson || {};
    const baselineData = contract.baselineJson || {};
    ...
    const terminalMeta = terminalEvent.metadataJson || {};
    const stripeRefs = {
 ...
 */
export function canSettle(currentState, events) {
    ...
    const verificationSucceeded = events.some(e => e.eventType === EventType.VERIFICATION_SUCCEEDED);
    const verificationFailed = events.some(e => e.eventType === EventType.VERIFICATION_FAILED);
 ...
 */
export async function settleContract(contractId, tx) {
    // 1. Get contract
    ...
    const terminalSettlementEvent = events.find(e => e.eventType === EventType.SETTLED_SUCCESS ||
    ...
    if (terminalSettlementEvent) {
        ...
        const metadata = terminalSettlementEvent.metadataJson;
        return {
            success: true,
    ...
    const hasSettlementStarted = events.some(e => e.eventType === EventType.SETTLEMENT_STARTED);
    ...
    try {
        ...
        const latestEvents = await getEventsForContract(contractId, tx);
        const terminalForReceipt = latestEvents.find(e => e.eventType === EventType.SETTLED_SUCCESS ||
            e.eventType === EventType.SETTLED_FAILURE);
        ...
        if (classified.retryable) {
            ...
            const nextRetry = await scheduleRetry(contractId, 'SETTLE', `${classified.category}: ${classified.originalError.message}`);
...
export async function runSettlementJob() {
    ...
    const verifiedContractIds = await db
        ...
        .from(contractIndex)
        .where(and(eq(contractIndex.currentState, ContractStatus.VERIFIED), eq(contractIndex.isTerminal, 0), or(isNull(contractIndex.nextRetryDueUtc), lte(contractIndex.nextRetryDueUtc, now))));
