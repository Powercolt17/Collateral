...
import { connectedAccounts, type Contract, type User, type ConnectedAccount } from '../db/schema.js';
...
export class MockXClient implements XClient {
    constructor(
        private fixedFollowers: number = 10000,  // Matches eligibility minimum; delta floor skipped in dev
        public mockBio: string = ''
    ) { }

    async getFollowers(_username: string): Promise<number> {
    ...
    async getUserWithHealth(_userId: string): Promise<XAccountHealth> {
...
export class RealXClient implements XClient {
    ...
    private async request<T>(endpoint: string, params?: Record<string, string>): Promise<T> {
        ...
        if (response.status === 401 || response.status === 403) {
            throw new XAdapterError(
                `X API auth error (${response.status}). User must reconnect X account.`,
                false,
                'AUTH'
            );
        }
        ...
        if (response.status >= 500) {
            throw new XAdapterError(
                `X API server error (${response.status})`,
                true,
                'API'
            );
        }
    ...
    async getUserByUsername(username: string): Promise<{ id: string; username: string } | null> {
        ...
        try {
            const result = await this.request<UserResponse>(`/users/by/username/${username}`);
            return result.data || null;
        } catch (err) {
            if (err instanceof XAdapterError) throw err;
            throw new XAdapterError(`Failed to get user: ${err}`, true, 'API');
        }
    }

    async getFollowers(usernameOrId: string): Promise<number> {
        ...
        const result = await this.request<UserMetricsResponse>(
            `/users/${userId}`,
    ...
    async getUserProfile(userId: string): Promise<{ description?: string } | null> {
        ...
        try {
            const result = await this.request<UserProfileResponse>(
                `/users/${userId}`,
    ...
    async getUserWithHealth(userId: string): Promise<XAccountHealth> {
        ...
        try {
            const result = await this.request<UserHealthResponse>(
                `/users/${userId}`,
...
export function getXClient(): XClient {
    ...
    throw new XAdapterError(
        'X not configured. Set X_API_BEARER_TOKEN or connect user OAuth.',
        false,
        'CONFIG'
    );
}

export function setXClient(client: XClient) {
...
export interface VerificationContext {
    windowStartUtc: Date;
    stripeConnectedAccountId?: string; // Not used for X but kept for interface consistency
}
...
async function getXConnectedAccount(userId: string): Promise<ConnectedAccount | null> {
 ...
 */
async function getVerifiedXConnectedAccount(userId: string): Promise<ConnectedAccount | null> {
...
export const xAdapter: SimplePlatformAdapter = {
    platform: 'X',
    ...
    async connect(user: User): Promise<ConnectedAccount> {
        ...
        throw new XAdapterError(
            'Use POST /v1/connect/x endpoint to connect X account by username.',
            false,
            'CONFIG'
        );
    },
    ...
    async snapshotBaseline(contract: Contract) {
        ...
        if (!xAccount) {
            throw new XAdapterError(
                `No VERIFIED X account for user ${contract.principalUserId}. ` +
                'User must connect and verify their X account before creating a contract.',
                false,
                'CONFIG'
            );
        }
        ...
        if (!eligibility.eligible) {
            throw new XAdapterError(
                `X account not eligible for contracts: ${eligibility.reasonCode}. ` +
                ...
                `tweets=${eligibility.details.tweetCount}`,
                false,
                'CONFIG'
            );
        }
        ...
        if (!deltaValidation.valid) {
            throw new XAdapterError(
                `Target threshold does not meet minimum delta floor. ` +
                `Baseline: ${followers}, Target: ${condition.threshold}, ` +
                `Required delta: ${deltaValidation.requiredDelta}, Actual delta: ${deltaValidation.actualDelta}`,
                false,
                'CONFIG'
            );
        }
    ...
    async evaluate(contract: Contract, context: VerificationContext) {
        const condition = contract.conditionJson as {
            operator: string;
            threshold: number;
        };

        const metricType = contract.metricType as string;
        const client = getXClient();
        ...
        if (!xAccount) {
            throw new XAdapterError(
                `No VERIFIED X account for user ${contract.principalUserId}. ` +
                'Cannot verify without verified connected account.',
                false,
                'CONFIG'
            );
        }
        ...
        if (metricType !== 'FOLLOWERS') {
            throw new XAdapterError(
                `Unsupported metric type for X: ${metricType}. Only FOLLOWERS is currently supported.`,
                false,
                'UNSUPPORTED'
            );
        }
        ...
        if (!baseline?.followers || !baseline?.deltaFloor) {
            throw new XAdapterError(
                'Missing frozen baseline data. Contract may not have been properly executed.',
                false,
                'CONFIG'
            );
        }
