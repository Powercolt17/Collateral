...
async function run() {
    // Mark all DRAFT creators as READY
    ...
    // Export all creator referral links
    ...
    let i = 1;
    for (const c of creators.rows || creators) {
        const cr = c as any;
    ...
    const alist = (creators.rows || creators).filter((c: any) => c.tier === 'A_LIST');
