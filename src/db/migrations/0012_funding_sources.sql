...
-- Status enum for funding sources (use DO block for IF NOT EXISTS)
DO $$ BEGIN
...
CREATE INDEX IF NOT EXISTS idx_funding_sources_stripe_customer ON funding_sources(stripe_customer_id);

-- Index for webhook processing by setup intent
CREATE INDEX IF NOT EXISTS idx_funding_sources_setup_intent ON funding_sources(stripe_setup_intent_id);
