...
export interface IdempotentParams {
    scope: string;  // e.g., 'stripe:webhook', 'stripe:payout', 'contract:settle'
    ...
    for (let attempt = 0; attempt < MAX_ACQUISITION_ATTEMPTS; attempt++) {
        ...
        switch (acquisitionResult.status) {
            ...
            case 'cached':
                ...
                return {
                    status: 'cached',
                    result: acquisitionResult.result as T,
...
interface AcquisitionResult {
    status: 'acquired' | 'cached' | 'in_progress' | 'contention';
    result?: unknown;
    completedAt?: Date | null;
}
...
async function getIdempotencyKey(key: string): Promise<IdempotencyKey | null> {
    ...
    return rows[0] as unknown as IdempotencyKey;
}

async function markSucceeded<T>(key: string, result: T): Promise<void> {
    await db.execute(sql`
        UPDATE idempotency_keys
...
async function markFailed(key: string, err: unknown): Promise<void> {
 ...
 */
export async function isIdempotentKeyCompleted(
    scope: string,
    key: string
): Promise<boolean> {
    const fullKey = `${scope}:${key}`;
