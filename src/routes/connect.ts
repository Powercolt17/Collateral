...
function isExpired(issuedAt: Date | null): boolean {
 ...
 */
function normalizeBioText(text: string): string {
 ...
 */
function bioContainsChallenge(bio: string, challengeCode: string): boolean {
 ...
 */
function maskChallengeCode(code: string): string {
...
async function connectRoutes(fastify: FastifyInstance) {
    ...
    fastify.post<{ Body: { username: string } }>(
        ...
        async (request, reply) => {
            ...
            let xUser: { id: string; username: string } | null;
            ...
            const updateResult = await db
                ...
                .where(
                    and(
                        ...
                        sql`${connectedAccounts.verificationStatus} != 'VERIFIED'`,
                        or(
                            isNull(connectedAccounts.challengeIssuedAt),
                            lt(connectedAccounts.challengeIssuedAt, cooldownThreshold)
                        )
                    )
                )
                .returning();
            ...
            if (updateResult.length === 0) {
                ...
                if (insertResult.length > 0) {
                    ...
                    if (current?.verificationStatus === 'VERIFIED') {
                        const meta = current.metadataJson as XConnectionMetadata | null;
                        return reply.status(200).send({
                            platform: 'X',
