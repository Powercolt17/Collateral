 ...
 */
export function assertNoUserIdFields(payload: unknown): void {
    ...
    for (const field of BANNED_USER_ID_FIELDS) {
        if (field in obj && obj[field] !== undefined && obj[field] !== null) {
            throw new AuthError(
                `Do not supply ${field}; user identity is derived from authentication.`,
                'USER_ID_NOT_ALLOWED'
            );
        }
    }
 ...
 */
export function getPrincipal(request: FastifyRequest): string {
    if (!request.principalUserId) {
 ...
 */
export function assertPrincipalMatch(
    request: FastifyRequest,
    payloadUserId: string | undefined | null
): void {
    const principal = getPrincipal(request);

    if (payloadUserId && payloadUserId !== principal) {
        throw new AuthError(
            `Payload userId mismatch: request attempted to act as ${payloadUserId}`,
            'USER_MISMATCH'
        );
    }
}
