...
export async function handlePaymentSuccess(
    ...
    const authorizedEventExists = await eventExistsForExternalRef(
        contractId,
        paymentIntentId,
    ...
    if (!authorizedEventExists) {
        throw new Error(
            `Untrusted payment binding: No FUNDS_AUTHORIZED event found for payment_intent ${paymentIntentId} on contract ${contractId}`
        );
    }
