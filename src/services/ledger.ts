 ...
 */
function computeEventHash(payload: HashPayload): string {
 ...
 */
async function getChainHead(contractId: string, txClient?: DbLike): Promise<LedgerEvent | null> {
 ...
 */
export async function eventExistsForExternalRef(
    contractId: string,
    externalRef: string,
    eventType?: string,
    txClient?: DbLike
    ...
    if (eventType) {
        conditions.push(eq(ledgerEvents.eventType, eventType as EventTypeType));
    }
 ...
 */
export async function getContractIdByPaymentIntent(
    ...
    const [event] = await db
        ...
        .where(
            and(
                eq(ledgerEvents.externalRef, paymentIntentId),
                eq(ledgerEvents.eventType, EventType.FUNDS_AUTHORIZED)
            )
        )
        .limit(1);
 ...
 * @param txClient - Optional transaction client for read-your-writes
 */
async function getEventByExternalRef(
    contractId: string,
    externalRef: string,
    eventType?: string,
    txClient?: DbLike
 ...
 */
export async function appendEvent(params: AppendEventParams): Promise<LedgerEvent> {
    ...
    try {
        ...
        const executeInsert = async (txClient: DbLike) => {
            ...
            const isTerminal = isStateChangingEvent && TERMINAL_STATES.has(derivedState as any) ? 1 : 0;

            let nextRetryDueUtc: Date | null = null;
            if (eventType === EventType.RETRY_SCHEDULED && metadata) {
                const retryMeta = metadata as { nextAttemptAtUtc?: string };
            ...
            if (isStateChangingEvent && TERMINAL_STATES.has(derivedState as any)) {
 ...
 */
export async function getEventsForContract(contractId: string, txClient?: DbLike): Promise<LedgerEvent[]> {
    const client = txClient ?? db;
    return client
        .select()
        .from(ledgerEvents)
 ...
 * Cursor format: "timestampIso|id"
 */
function createCursor(event: LedgerEvent): string {
 ...
 */
export async function getLedgerEvents(options: PaginationOptions = {}): Promise<PaginatedEvents> {
    ...
    if (options.cursor) {
        ...
        if (parsed) {
            ...
            events = await db
                ...
                .where(
                    or(
                        lt(ledgerEvents.timestampUtc, parsed.timestamp),
                        and(
                            eq(ledgerEvents.timestampUtc, parsed.timestamp),
                            lt(ledgerEvents.id, parsed.id)
                        )
                    )
                )
                .orderBy(desc(ledgerEvents.timestampUtc), desc(ledgerEvents.id))
 ...
 */
export async function verifyHashChain(contractId: string): Promise<boolean> {
