...
import { runIdempotent, IdempotencyScope } from '../invariants/idempotency.js';
 ...
 */
async function findStuckSettlement(): Promise<string[]> {
    ...
    const result = await db
        ...
        .where(
            and(
                or(
                    eq(contractIndex.currentState, 'VERIFICATION_SUCCEEDED'),
                    eq(contractIndex.currentState, 'SETTLEMENT_STARTED'),
                    eq(contractIndex.currentState, 'SETTLING')
                ),
                lt(contractIndex.lastEventAtUtc, threshold),
                eq(contractIndex.isTerminal, 0),
                or(
                    isNull(contractIndex.nextRetryDueUtc),
                    lte(contractIndex.nextRetryDueUtc, new Date())
                )
            )
        );
 ...
 */
async function findPayoutDeferred(): Promise<{ contractId: string; principalUserId: string }[]> {
    const result = await db
        .select({
            contractId: contractIndex.contractId,
        ...
        .where(
            and(
                or(
                    eq(contractIndex.currentState, 'PAYOUT_DEFERRED'),
                    eq(contractIndex.currentState, 'PAYOUT_PENDING')
                ),
                or(
                    isNull(contractIndex.nextRetryDueUtc),
                    lte(contractIndex.nextRetryDueUtc, new Date())
    ...
    const deferred: { contractId: string; principalUserId: string }[] = [];
 ...
 */
async function findMissingReceipts(): Promise<string[]> {
 ...
 */
async function reDriveSettlement(contractId: string): Promise<{ success: boolean; cached: boolean }> {
    const dayBucket = getDayBucket();
    const idempotencyKey = `settle:${contractId}:${dayBucket}`;

    try {
        const result = await runIdempotent(
            { scope: 'reconcile:settle', key: idempotencyKey },
            async () => {
 ...
 * to ensure all DB writes use the lock-pinned connection.
 */
async function reDriveVerification(contractId: string): Promise<{ success: boolean; cached: boolean }> {
    const dayBucket = getDayBucket();
    const idempotencyKey = `verify:${contractId}:${dayBucket}`;

    try {
        const result = await runIdempotent(
            { scope: 'reconcile:verify', key: idempotencyKey },
            async () => {
                return await withContractLock(contractId, async (tx) => {
                    // Lock acquired inside transaction - tx MUST be used for writes
 ...
 */
export async function reconcileSweep(): Promise<ReconciliationResult> {
 ...
 */
export async function processDeferredPayouts(): Promise<{
    processed: number;
    succeeded: number;
    failed: number;
    errors: string[];
 ...
 */
export async function runReconciliationJob(): Promise<{
    processed: number;
    recovered: number;
    stillStuck: number;
    skipped: number;
