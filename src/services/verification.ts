...
import { contracts, ContractStatus, EventType, type Contract, users, contractIndex } from '../db/schema.js';
...
// =====================================================
// VERIFICATION_STARTED  → state becomes VERIFYING
// VERIFICATION_SUCCEEDED → state becomes VERIFIED (outcome = pass)
// VERIFICATION_FAILED    → state becomes VERIFIED (outcome = fail)
 ...
 */
export async function verifyContract(contractId: string, tx?: DbLike): Promise<VerificationResult> {
    // 1. Get contract
    ...
    const terminalVerificationEvent = events.find(e =>
        e.eventType === EventType.VERIFICATION_SUCCEEDED ||
        e.eventType === EventType.VERIFICATION_FAILED
    );
    if (terminalVerificationEvent) {
        const metadata = terminalVerificationEvent.metadataJson as Record<string, any>;
        return {
            success: true,
    ...
    const hasVerificationStarted = events.some(e =>
        e.eventType === EventType.VERIFICATION_STARTED
    );
    ...
    try {
        ...
        // 7. Call Adapter
        let evalResult: VerificationResult;
        ...
        if (contract.platform === 'X') {
            ...
            evalResult = {
                ...
                evidence: xResult.evidence as any
            };
            ...
            evalResult = {
                ...
                evidence: stripeResult.evidence as any
            };
        ...
        return {
            ...
            threshold: evalResult.threshold,
            operator: evalResult.operator,
            evidence: evalResult.evidence as VerificationResult['evidence'],
        };
    } catch (err: any) {
 ...
 */
async function appendContractVerifiedIfMissing(contractId: string, pass: boolean, tx?: DbLike): Promise<void> {
    const events = await getEventsForContract(contractId, tx);
...
export async function runVerificationJob(): Promise<{
    ...
    const eligibleContractIds = await db
        ...
        .where(
            and(
                eq(contractIndex.currentState, ContractStatus.LOCKED),
                eq(contractIndex.isTerminal, 0),
                lte(contractIndex.deadlineUtc, now),
                or(
